...
def _scrape_gmp_report(status_filter: Optional[str] = None) -> list[IPORecord]:
    ...
    for row in rows:
        # Prefer the explicit machine-readable category field when present,
        # fall back to parsing the visible badge out of the Name HTML.
        category_text = row.get("~IPO_Category") or ""
        company_name, badge_category, status_text = _parse_name_cell(row.get("Name", ""))
        ...
        open_d = _parse_date(row.get("Open"))
        close_d = _parse_date(row.get("Close"))
        boa_d = _parse_date(row.get("BoA Dt"))
        listing_d = _parse_date(row.get("Listing"))
